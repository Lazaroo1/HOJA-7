...
public void cargarDesdeCSV(String archivo) throws IOException {
    try (BufferedReader br = new BufferedReader(new FileReader(archivo))) {
        ...
        while ((linea = br.readLine()) != null) { 
            ...
            if (datos.length >= 3) {  // Mínimo tenemos que tener: SKU, Nombre, Descripción
                ...
                // Procesar tallas si existen ( están en la columna 4)
                if (datos.length > 3) {
                    String[] tallas = datos[3].split("\\|");
                    for (String tallaInfo : tallas) {
                        String[] partes = tallaInfo.split(":");
                        if (partes.length == 2) {  // para ver si estan en el formato correcto de talla:cantidad
